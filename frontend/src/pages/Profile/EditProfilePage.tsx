...
import { ArrowLeft, User, Upload, Loader2, ImageOff } from "lucide-react";
...
export default function EditProfilePage() {
  ...
  // fileInputRef kullanılmıyor gibi görünse de aşağıdaki iç bileşende kullanılacak; bu satır kaldırılacak.
}
...
function EditProfileFormBody({
  ...
  return (
    <div className="container edit-profile-page">
      ...
      <form className="edit-profile-page__card" onSubmit={handleSubmit}>
        <div className="edit-profile-page__header">
          <p className="edit-profile-page__eyebrow">{t("profile.member")}</p>
        ...
        <div
          className={`edit-profile-page__dropzone ${isDragging ? "is-dragging" : ""}`}
        ...
        >
          ...
          <div className="edit-profile-page__avatar-preview">
            {uploadMutation.isPending ? (
              <Loader2 size={22} className="edit-profile-page__spinner" />
            ) : previewUrl ? (
          ...
          <p className="edit-profile-page__dropzone-text">
            <Upload size={13} />{" "}
          ...
          {previewUrl && !uploadMutation.isPending && (
            <button
              type="button"
              className="edit-profile-page__remove-avatar"
              onClick={(e) => {
                e.stopPropagation();
                handleRemoveAvatar();
              }}
